...
import madfox.colhh.jokessender.SendProgressFragment.SendingResult;
...
public class SendFragment extends Fragment implements OnTouchListener,SendingResult {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		View v=getView();
		txt_phone=(EditText)v.findViewById(R.id.send_txt_phone);
		txt_quote=(EditText)v.findViewById(R.id.send_txt_quote);
		btn_send=(TextView)v.findViewById(R.id.send_btn_send);
		btn_schedule=(TextView)v.findViewById(R.id.schedule_btn_send);
		...
		}else
		{
			if(getResources().getConfiguration().orientation==Configuration.ORIENTATION_PORTRAIT)
			{
				...
				.replace(R.id.add_layout_adz, adf=new AdsFlasher(3000,0,
						Packages.QUOTES_SENDER,Packages.BD_WISHES_SENDER,
		...
		switch(v.getId())
		{
			case R.id.send_btn_send:
				...
				if(event.getAction()==MotionEvent.ACTION_DOWN)
				{
					...
					btn_send.setTextColor(Color.rgb(255,68,68));
					return true;
				}else if(event.getAction()==MotionEvent.ACTION_UP)
				{
					btn_send.setBackgroundColor(Color.rgb(255,68,68));
	...
	public void onSendingCompleted(boolean result) {
		...
		if(result)
		{
			Toast toast=Toast.makeText(getActivity(), "Joke Sent Successfully...", Toast.LENGTH_LONG);
		...
		{
			Toast toast=Toast.makeText(getActivity(), "Joke Sending failed, Please resend...", Toast.LENGTH_LONG);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
			//add show
			MobileCore.showInterstitial(activity, null);
